			...
			for y, data in pairs(items) do
				...
				if (item and !IsValid(self.panels[item.id])) then
					local icon = self:AddIcon(item:GetModel() or "models/props_junk/popcan01a.mdl",
						x, y, item.width, item.height, item:GetSkin(), item:GetModelBodygroups())
...
function PANEL:BuildSlots()
	...
	for x = 1, self.gridW do
		...
		for y = 1, self.gridH do
			...
			slot.gridX = x
			slot.gridY = y
			slot:SetPos((x - 1) * iconSize, (y - 1) * iconSize + self:GetPadding(2))
...
function PANEL:PaintDragPreview(width, height, mouseX, mouseY, itemPanel)
	...
	if (item) then
		...
		local dropX = math.ceil((mouseX - 0 - (itemPanel.gridW - 1) * 32) / iconSize)
		...
		for x = 0, itemPanel.gridW - 1 do
			for y = 0, itemPanel.gridH - 1 do
				...
				surface.DrawRect((x2 - 1) * iconSize, (y2 - 1) * iconSize + self:GetPadding(2), iconSize, iconSize)
			end
		end
	end
end
...
function PANEL:Think()
	if ix.gui.menuInventoryParent then
		if ix.gui.menuInventoryParent.backpacks then
			local backpacks = ix.gui.menuInventoryParent.backpacks

			local inventory = LocalPlayer():GetCharacter():GetInventory()
			if !ix.gui.menuInventoryParent.backpacks.slot then
				if (!inventory:HasItem("smallbag") and !inventory:HasItem("largebag")) then
					for x = 1, 7 do
						for y = 1, 4 do
							backpacks.slot = ix.gui.menuInventoryParent.backpacks:Add("Panel")
							backpacks.slot:SetPos((x - 1) * iconSize, (y - 1) * iconSize + SScaleMin(30 / 3))
							backpacks.slot:SetSize(iconSize, iconSize)

							backpacks.slot.Paint = PaintLockedSlot
						end
					end
				end
			end

			if !ix.gui.menuInventoryParent.backpacks.noLargeslot then
				if inventory:HasItem("smallbag") and !inventory:HasItem("largebag") then
					for x = 1, 4 do
						for y = 1, 4 do
							backpacks.noLargeslot = ix.gui.menuInventoryParent.backpacks:Add("Panel")
							backpacks.noLargeslot:SetPos((x - 1) * iconSize + (iconSize * 3), (y - 1) * iconSize + SScaleMin(30 / 3))
							backpacks.noLargeslot:SetSize(iconSize, iconSize)

							backpacks.noLargeslot.Paint = PaintLockedSlot
						end
					end
				end
			end

			if !ix.gui.menuInventoryParent.backpacks.noSmallslot then
				if inventory:HasItem("largebag") and !inventory:HasItem("smallbag") then
					for x = 1, 3 do
						for y = 1, 4 do
							backpacks.noSmallslot = ix.gui.menuInventoryParent.backpacks:Add("Panel")
							backpacks.noSmallslot:SetPos((x - 1) * iconSize + (iconSize * 4), (y - 1) * iconSize + SScaleMin(30 / 3))
							backpacks.noSmallslot:SetSize(iconSize, iconSize)

							backpacks.noSmallslot.Paint = PaintLockedSlot
						end
					end
				end
			end
...
hook.Add("CreateMenuButtons", "ixInventory", function(tabs)
	...
	tabs["inv"] = {
		...
		Create = function(info, tempCont)
			...
			for _, v in SortedPairsByMemberValue(inventory:GetItemsByBase("base_bags", true), "id") do
				if (!v.isBag) then
					continue
				end

				if (v.uniqueID != "largebag" and v.uniqueID != "smallbag") then
					continue
				elseif (v.name == "largebag") then
					if (bFoundLarge) then
						continue
					else
						bFoundLarge = true
					end
				elseif (v.name == "smallbag") then
					if (bFoundSmall) then
						continue
					else
						bFoundSmall = true
