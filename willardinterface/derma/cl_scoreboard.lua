...
function PANEL:Update()
	...
	if (!bRecognize) then
		self.icon.material = Material("willardnetworks/tabmenu/scoreboard/question.png", "smooth nocull")
	end
	...
	if (self.name:GetText() != name) then
		self.name:SetText(bRecognize and name or L("неизвестно"))
...
function PANEL:SetFaction(faction)
	...
	self.Paint = function(self, w, h)
		...
		surface.DrawRect(1, 0, self:GetWide() - 2, SScaleMin(31 / 3))
	end
end
...
function PANEL:Update()
	...
	for k, v in pairs(player.GetAll()) do
		if (v:GetCharacter()) then
			local playerFaction = ix.faction.Get(v:GetCharacter():GetFaction())
			local separateTab = playerFaction.separateUnknownTab

			if v.group and !v.group.members[v:GetCharacter():GetID()] then
				v.group = nil
			end
			
			v.unknown = !separateTab and !LocalPlayer():GetCharacter():DoesRecognize(v:GetCharacter())
	...
	if (self.group) then
		...
		for k, v in pairs(self.faction:GetOnlineMembers()) do
...
function PANEL:CreateTitleText()
	...
	scoreboardTitle:SetText("Список игроков")
	scoreboardTitle:SizeToContents()
