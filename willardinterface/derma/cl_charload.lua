...
function PANEL:Init()
	...
	subtitleLabel:SetText("Выберете персонажа")
	...
	if self.CharacterCount <= 4 then
		panelLoad:SetSize((self.CharacterCount) * charPanelW + ((self.CharacterCount - 1) * margin), titleLabel:GetTall() + subtitleLabel:GetTall() + SScaleMin(590 / 3) + (margin * 2) + SScaleMin(30 / 3))
	else
		panelLoad:SetSize((4) * charPanelW + ((4 - 1) * margin), titleLabel:GetTall() + subtitleLabel:GetTall() + SScaleMin(590 / 3) + (margin * 2) + SScaleMin(30 / 3))
	end
	...
	if self.CharacterCount > 4 then
		...
		self.nextButton.DoClick = function()
			...
			if math.Round(math.abs( x )) == math.Round(((self.CharacterCount - 5) * (charPanelW + margin))) then
			...
			if IsValid(self.previousButton) then
				...
				local x, y = self.previousButton:GetPos()
				self.previousButton:MoveLeftOf(panelLoad)
				local x2, y2 = self.previousButton:GetPos()
				...
				self.previousButton.DoClick = function()
					...
					if IsValid(self.previousButton) then
						if math.Round(x, 0) == math.Round((0 - (charPanelW + margin)), 0) then
	...
	for i = 1, #ix.characters do
		...
		nameText:SetTextColor(Color(0, 0, 0, 255))

		local factionText = self.characterTextPanel:Add("DLabel")
		factionText:SetFont("WNMenuFontNoClamp")
		if (faction.name) == "Сверхчеловеческий Патруль" then
			factionText:SetText("СОЛДАТ ПАТРУЛЯ")
		else
		...
		factionText:SetTextColor(Color(0, 0, 0, 255))
...
/*
	if ix.config.Get("CharCreationDisabled", false) and #ix.characters == 0 then
        Derma_Query( "Do you want to connect to the city server?", "You don't have any characters at this location.", "ДА", function()
...
*/
	...
	backPanel:DockMargin(0, margin, 0, 0)

	local back = backPanel:Add("DButton")
	back:SetText("Назад")
	back:SetContentAlignment(6)
	back:SetSize(SScaleMin(80 / 3), SScaleMin(30 / 3))
	back:SetTextColor(Color(230, 230, 230, 255))
	back:SetFont("WNBackFontNoClamp")
	back:Center()
	back:SetTextInset(SScaleMin(10 / 3), 0)
...
-- character deletion panel
	...
	deleteTitle:SetText(string.utf8upper("вы уверены?"))
	deleteTitle:SetTextColor(Color(0, 0, 0, 255))
	...
	deleteText:SetText("Этот персонаж будет безвозвратно удален!")
	deleteText:SetTextColor(color_black)
	...
	yes:SetText(string.utf8upper("да"))
	...
	no:SetText(string.utf8upper("нет"))
