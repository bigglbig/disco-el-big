...
function PANEL:Paint(width, height)
	...
	if (amount > 0) then
		local color = Color(255, 255, 255, amount)
...
function PANEL:Init()
	...
	local logo = ix.util.GetMaterial("aftermath/logo.png")

	local logoMargin = SScaleMin(17 / 3)
	local logoImage = logoPanel:Add("DImage")
	logoImage:SetMaterial(logo)
	logoImage:SetSize(SScaleMin(808 / 5), SScaleMin(404 / 5))
	logoImage:Dock(TOP)
	logoImage:DockMargin(logoPanel:GetWide() * 0.5 - logoImage:GetWide() * 0.5, 0, logoPanel:GetWide() * 0.5 - logoImage:GetWide() * 0.5, logoMargin)
	...
	self.buttonList:DockMargin(logoPanel:GetWide() * 0.5 - self.buttonList:GetWide() * 0.5, 0, logoPanel:GetWide() * 0.5 - self.buttonList:GetWide() * 0.5, 0)
...
function PANEL:CreateButtons()
	...
	local buttonHalfHeight = buttonHeight * 0.5
	local iconWidth = SScaleMin(20 / 3)
	local iconHeight = iconWidth
	local iconHalfHeight = iconHeight * 0.5

	local bHasCharacter = #ix.characters > 0
	self.bUsingCharacter = LocalPlayer().GetCharacter and LocalPlayer():GetCharacter()

	local function PaintButton(parent, icon, text, boolLoad, charCreationDisabled)
		parent:SetText(text)
		parent:SetFont("WNMenuFontNoClamp")
		parent:SetTextColor(Color(0, 0, 0, 255))
		parent:SetContentAlignment(4)
		parent:SetTextInset(iconWidth + SScaleMin(15 / 3), 0)
		parent:DockMargin(0, 0, SScaleMin(5 / 3), 0)
		self.buttonWidth = self.buttonWidth + SScaleMin(5 / 3)
		parent.iconColor = Color(0, 0, 0, 255)

		if (!bHasCharacter and boolLoad) or charCreationDisabled then
			parent:SetDisabled(true)
			parent:SetTextColor(Color(90, 90, 90, 255))
		end

		if charCreationDisabled then
			parent.iconColor = Color(90, 90, 90, 255)
		end
		...
		parent.Paint = function(self, w, h)
			if boolLoad then
				...
				if self:IsHovered() and !charCreationDisabled then
					draw.RoundedBox( 10, 0, 0, w, h, Color(78, 79, 100, 240) )
				end
			end
			...
			if !boolLoad then
				...
				surface.DrawTexturedRect(SScaleMin(9 / 3), buttonHalfHeight - iconHalfHeight, iconHeight, iconHeight)
			end
		end
	end
	...
	local function OnCursor(name, boolLoad, charCreationDisabled)
		name.OnCursorEntered = function()
			...
			surface.PlaySound("aftermath/ui/gui_journal_toggle_r005_01.ogg")
			name:SetTextColor(Color(255, 255, 255, 255))
		end
		...
		name.OnCursorExited = function()
			...
			if charCreationDisabled then
				name:SetTextColor(Color(90, 90, 90, 255))
				return
			end
	...
	PaintButton(createButton, "willardnetworks/mainmenu/new_arrival.png", "НОВОЕ ПРИБЫТИЕ", nil, ix.config.Get("CharCreationDisabled", false))
	OnCursor(createButton, nil, ix.config.Get("CharCreationDisabled", false))
	...
	PaintButton(self.loadButton, "", "ПЕРСОНАЖИ", true)

	self.loadButtonIcon = self.loadButton:Add("DImage") -- Don't know why the fuck it doesn't work properly without this but ok
	if bHasCharacter then
		self.loadButtonIcon:SetImageColor(Color(0, 0, 0, 255))
	else
		self.loadButtonIcon:SetImageColor(Color(90, 90, 90, 255))
	end

	self.loadButtonIcon:SetSize(iconWidth, iconHeight)
	self.loadButtonIcon:SetPos(SScaleMin(9 / 3), buttonHalfHeight - iconHalfHeight)
	...
	PaintButton(communityButton, "willardnetworks/mainmenu/info.png", "ИНФОРМАЦИЯ")
	...
	communityButton.DoClick = function()
		gui.OpenURL("https://discord.gg/24hX9JuzU8")
		surface.PlaySound("aftermath/ui/gui_journal_windowin_r001_01.ogg")
	end
	...
	PaintButton(contentButton, "willardnetworks/mainmenu/content.png", "КОНТЕНТ")
	...
	contentButton.DoClick = function()
		gui.OpenURL("https://steamcommunity.com/sharedfiles/filedetails/?id=3252404886")
		surface.PlaySound("aftermath/ui/gui_journal_windowin_r001_01.ogg")
	end
	...
	PaintButton(self.returnButton, "willardnetworks/mainmenu/exit.png", "")
...
function PANEL:UpdateReturnButton(bValue)
	...
	self.returnButton:SetText(bValue and "НАЗАД" or "ВЫХОД")
	self.returnButton:SizeToContents()
end
...
function PANEL:Paint(width, height)
	if ix.gui.background_url then
		...
		surface.SetDrawColor(Color(90, 0, 0, 100))
		surface.DrawRect(0, 0, width, height)
